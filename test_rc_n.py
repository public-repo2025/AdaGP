...
from utils import get_rect_p, get_timestamps, budget_rho_rem, Bvec_init, get_traj_dt, select_traj_dt
...
hs = [6,9,12,15,18,21,0,3]

run_range = range(0,50)
tick = time()
print('Starting test_rc_taxi_sq_n for range'+str(run_range[0])+' - '+str(run_range[-1]))
...
for jrep in run_range:
    ...
    seed_n = seeds_rep[jrep]
    dict_traj_rep = select_traj_dt(dict_traj,nj,seed=seed_n)
    for j in range(len(ws)):
        ...
        test_rc(dict_traj_rep,times,B,seed,ps, rect_w=w,rect_l=l,params_in=params_in,mechs=mechs,folder_out0=folder_out)
