...
r_end = len(run_range)
tick = time()
print('Starting test_knn_taxi_n for range'+str(run_range[0])+' - '+str(run_range[-1]))
...
for jrep in run_range:
    ...
    dict_traj_rep = select_traj_dt(dict_traj_dt,nj,seed=seed_n)
    for j in range(len(ks)):
        ...
        test_knn(dict_traj_rep,times,k,B,seed,ps,params_in=params_in,mechs=mechs,folder_out0=folder_out)
